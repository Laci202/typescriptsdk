...
export interface GetPdfJobStatusRequest {
    jobId: string;
}
...
export interface PDFAsyncApiInterface {
     ...
     * @memberof PDFAsyncApiInterface
     */
    getPdfJobStatusRaw(requestParameters: GetPdfJobStatusRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<JobStatusResponse>>;

    /**
     * Get the current status of an async PDF generation job.  **Authentication:** API Key required (`x-api-key` header)  ## Status Values  | Status | Description | |--------|-------------| | `pending` | Job is queued, waiting to be processed | | `processing` | Job is being processed | | `completed` | PDF generated successfully | | `failed` | Job failed (check error_message) |  ## Polling Recommendations  - Poll every 1-2 seconds for small documents - Poll every 5-10 seconds for large documents - Consider using webhooks instead of polling
     * Get PDF job status
     */
    getPdfJobStatus(jobId: string, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<JobStatusResponse>;
...
export class PDFAsyncApi extends runtime.BaseAPI implements PDFAsyncApiInterface {
     ...
     * Get PDF job status
     */
    async getPdfJobStatusRaw(requestParameters: GetPdfJobStatusRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<JobStatusResponse>> {
        if (requestParameters['jobId'] == null) {
            throw new runtime.RequiredError(
                'jobId',
                'Required parameter "jobId" was null or undefined when calling getPdfJobStatus().'
            );
        }
        ...
        let urlPath = `/v1/pdf/status/{job_id}`;
     ...
     * Get PDF job status
     */
    async getPdfJobStatus(jobId: string, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<JobStatusResponse> {
        const response = await this.getPdfJobStatusRaw({ jobId: jobId }, initOverrides);
        return await response.value();
    }
